...
-- Functional extensionality blocks computation
def f (x : Nat) := x
def g (x : Nat) := 0 + x
...
def val2 : Nat := Eq.recOn (motive := fun _ _ => Nat) tteq 0
#reduce val2 -- does not reduce to 0
#eval val2 -- evaluates to 0
...
axiom Quot.sound :
  ∀ {α : Type u} {r : α → α → Prop} {a b : α}, r a b → Quot.mk r a = Quot.mk r b
end Hidden

def mod7Rel (x y : Nat) : Prop := x % 7 = y % 7
#check (Quot mod7Rel : Type) -- the quotient type
#check (Quot.mk mod7Rel 4 : Quot mod7Rel) -- the class of a

def f1 (x : Nat) : Bool := x % 7 = 0
theorem f1_respects (a b : Nat) (h : mod7Rel a b) : f1 a = f1 b := by
  simp [mod7Rel, f1] at *
  rw [h]

#check (Quot.lift f1 f1_respects : Quot mod7Rel → Bool)

-- the computation principle
example (a : Nat) : Quot.lift f1 f1_respects (Quot.mk mod7Rel a) = f1 a := rfl
...
def UProd (α : Type u) : Type u := Quotient (uprodSetoid α)

namespace UProd
  def mk {α : Type} (a₁ a₂ : α) : UProd α := Quotient.mk' (a₁, a₂)
  notation "{ " a₁ ", " a₂ " }" => mk a₁ a₂
...
private theorem mem_swap {a : α} :
  ∀ {p : α × α}, mem_fn a p ↔ mem_fn a (⟨p.2, p.1⟩) -- changed = to ↔
  | (a₁, a₂) => by
    -- apply propext -- commented this out
    apply Iff.intro
    . intro
...
axiom choice {α : Sort u} : Nonempty α → α -- blocks computation
...
noncomputable
def inhabited_of_nonempty : Nonempty α → Inhabited α :=
  fun h => choice (let ⟨a⟩ := h; ⟨⟨a⟩⟩)
end Hidden

open Classical
#check (@strongIndefiniteDescription :
  {α : Sort u} → (p : α → Prop) → Nonempty α → {x // (∃ (y : α), p y) → p x})
...
namespace Hidden
theorem em (p : Prop) : p ∨ ¬p := -- Diaconescu's theorem using Axiom of Choice
